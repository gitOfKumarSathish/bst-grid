...
| F2 | Disable row | ✅ built | P2 — rowDisabled | `enableCellSelection`, `rowDisabled` |
...
| I4 | Backend updates → cells/rows/grid | 🟡 partial | change-set half done v0.30.0 — batch mode's runtime.getChangeSet() + one onSave(&#123; changes, rows[].patch, next &#125;) per save action, with a rejected save keeping every draft; reconciling the backend's response back into cells/rows (server-authoritative values, partial failures) not built | `enableEditing` |
