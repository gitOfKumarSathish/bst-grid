...
export const RULES: Record<string, FlagRule> = {
  ...
  enableNotes: {
    note: 'Attach comments / annotations to cells without mutating row data. Cells with notes render a corner dog-ear marker; hover shows a preview popover; Shift+F2 opens the resizable note editor; context menu has Add/Edit/Delete note.',
  },
  enableFloatingActionBar: {
    requires: ['enableRowSelection'],
    note: 'Renders a floating bottom action toolbar when rows are selected with actions to Copy, Delete, Export, or Deselect.',
  },
  enableFloatingRowActions: {
    note: 'Hovering over a row shows a floating quick-action pill at the right edge of the row.',
  },
  enableFind: {
