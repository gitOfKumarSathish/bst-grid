...
right-side sheet of feature toggles. Choices are **per table** and saved to `localStorage`, so
they survive reloads. Only features you've provisioned are listed (default-on data features plus
any opt-in feature you enabled), so users can't switch on something the grid isn't wired for.
