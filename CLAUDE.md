...
> it is exactly the control Kagami is paying for by not licensing a commercial grid.
...
| A4 Nested / Master-Detail        | **Partial**            | Expanding + `getSubRows`/`renderSubComponent` gives state OOTB; the detail panel (esp. *different columns*) = nested table instance, custom render. **(Paid-tier elsewhere; free here.)**                                                                                                                                                                                                                                                                               |
...
- **Clipboard plugin** layered on the selection feature. **(Paid-tier elsewhere; free here.)**
...
*Avoid:* Highcharts (commercial), commercial enterprise grids, MUI X Pro/Premium (per-seat).
Their free tiers are MIT but lack master-detail/range-select/clipboard — the very features
...
> **Extended capability roadmap (2026-08-13).** The gaps below were audited against what commercial
> enterprise grids offer and are scheduled by phase — see `Plan.md` PART 3 "Phases 5–8", the `X1–X29`
> matrix in `COVERAGE.md`, and the full analysis in `docs/capability-roadmap.md`. Bst-Table already
> covers most **paid-tier** capabilities free; the biggest remaining hole is **export (CSV/Excel/print)**
...
- **Tree data** (self-referencing hierarchy) — clarify vs master-detail. *(X13 — ⚪ out of scope 2026-08-24; see COVERAGE.md "Optional".)*
...
| Page-size choices | `pageSizeOptions` | chrome | `(number \| 'all')[]` | [5,10,20,50] | adapter select — each entry is a numeric page size, or the string `'all'` for an **All** choice that shows every (filtered) row. Resolved by the engine's pure `resolvePageSizeChoices` / `pageSizeForChoice` so both skins behave identically | ✅ done |
...
| Cell/range selection + keyboard nav | `enableCellSelection` | engine | boolean | false | custom feature — **v9 ships `cellSelectionFeature`**, but ours stays: it is fused with keyboard nav, clipboard, type-to-edit, find, the context menu, notes (Shift+F2), the floating action bar and selection stats. Swapping the state layer would re-plumb all of them | ✅ done (Phase 3) |
| Type-to-edit (C5/C6) | `enableTypeToEdit` | engine | boolean | false | custom — spreadsheet-style entry: a printable keystroke on a selected cell opens the editor **seeded with that char** (overwrite, via the cell type `parse`), then **Enter commits & moves down** / **Tab commits & moves right** (⇧ reverses each). Needs `enableEditing` + `enableCellSelection` (resolved on the runtime handle so it's true only with both on); a no-op otherwise. Seeds only non-overlay editors that expose `parse` (text/number) and declines keys the type can't represent (a letter in a number cell → NaN). Commit-and-move routes through the existing `commitCell` (no new write path). In settings ("Editing", `requires` both prereqs). Both adapters inherit it (engine-level behaviour) | ✅ done |
...
| Column hide/show control | (in columns menu) | chrome | — | follows `enableHiding` | adapter columns menu — per-column **eye / eye-off** toggle (aria `Hide <col>` / `Show <col>`) beside pin/reorder/copy/edit, alongside the existing row visibility checkbox; calls `col.toggleVisibility()`. Hidden columns stay listed (menu iterates `getAllLeafColumns`) so they can be restored. shadcn adds `eye`/`eyeOff` icon slots (all presets) | ✅ done |
...
| Cell spanning (A5) | `enableCellSpanning` (+ `getCellSpan` / `meta.rowSpan:'group'`) | engine | boolean | false | custom render feature — `colSpan` **and** `rowSpan`; covered cells skipped (Plan.md §2.7a). **v9 does ship `cellSpanningFeature`** (9.1.2, in `stockFeatures`) — we deliberately keep ours: it is fused with the virtualization bypass (`virtualizationBypassReason`) and the `meta.rowSpan:'group'` contiguity rules. Do not re-plan this from "v9 lacks it" | ✅ done |
...
| Auto row height (X26) | `enableAutoRowHeight` (+ `meta.wrapText`) | engine | boolean | false | custom — **browser-measured, dep-free**: body cells `white-space: normal` so each row grows to fit wrapped content (root class `bst-auto-rowheight`; per-column `meta.wrapText` → `bst-wrap`). Keeps `overflow:hidden`, so a manually-resized row still clips to its set height. In settings ("Rows", always shown) | ✅ done |
...
| In-cell PDF thumbnail (B5) | `cellMeta.pdfThumbnail` | engine | boolean \| `PdfThumbnailRenderer` | false | `files` read cell renders **page 1 of a PDF** as a raster `<img>` thumbnail via **pdf.js** (§6's named tool — the native `<iframe>` won't paint a PDF at thumbnail size). The engine stays **dep-free**: it never imports pdf.js — the app injects a renderer with **`createPdfjsThumbnailer(pdfjs)`** through **`<BstPdfThumbnailerProvider>`** (or per-column: `cellMeta.pdfThumbnail` = a renderer fn). Shows the file-type icon while rendering / if no renderer / on failure. A server raster (`thumbnailUrl`) still wins. The full-size click **preview** (`BstFilePreview`) still uses the native `<iframe>` (with `data:`→`blob:` for Chrome). Opt-in per column; both skins inherit it. Not a settings-sheet toggle (a `cellMeta` option) | ✅ done |
...
| Sticky-header viewport (G3/G4) | `enableStickyHeader` | engine | boolean \| {maxHeight,maxRows} | false | custom render — caps the scroll box to a bounded height (`--bst-max-height`; default 440px, or `maxHeight` px/CSS length, or `maxRows` × row estimate) so the body scrolls under a **sticky header + filter row** instead of the table growing taller as the page size grows (`resolveStickyHeader` pure helper; class `bst-sticky-header`). Row virtualization already does this, so the standalone class is skipped when `enableVirtualization` is on (the two never stack). In settings ("Rows", always shown so an end-user can switch it on) | ✅ done |
...
| Grid-state save/restore (X21) | engine: `getGridState` / `applyGridState` / `resetGridState` / `loadGridState` / `saveGridState` / `clearGridState` / `useBstGridState` · adapters: `gridState={{ key }}` | engine API + adapter prop | functions + hook · `BstGridStateOptions` | — (opt-in) | serializable **view-state snapshot** (`BstGridState`: sort · filter · globalFilter · column order/size/visibility/pinning · grouping · expanded · rowPinning · rowSelection · pagination) for **per-user views** that survive reloads. v9 reads `table.store.state`, restores via per-slice setters; `applyGridState` **drops stale column ids** so a snapshot survives a column-set change. `include`/`exclude` narrow the slices; version-stamped (`BST_GRID_STATE_VERSION`) so incompatible blobs are ignored on load. Adapters' one-line **`gridState={{ key }}`** seeds `initialState` from storage + persists changes (a null-rendering child, so non-users pay nothing). In **manual mode** (`persist: false` + `showSettings`), the adapters also surface **Save view** / **Reset view** buttons in the settings-sheet footer (Save shown only when `persist:false`; Reset whenever `gridState` is set) under a **"Saved view"** section heading — no new flag, chrome only. **Two stores:** the sheet's feature toggles persist **automatically** to `bst-table:settings:<key>`, the view to `bst-table:state:<key>`; Save view does **not** capture feature toggles and each half has its own Reset (the captions say which is which). Distinct from `showSettings` (which toggles *features*). Not a settings-sheet toggle (an API + hook + config prop, like the server `DataSource`) | ✅ done |
...
| Log committed cell edits (debug) | `enableEditLog` (+ `onCellCommit`) | engine | boolean | false | custom — a debug companion to the **`onCellCommit(change)`** callback (itself the inline counterpart to `onSave`). `onCellCommit` fires once per committed cell the moment an edit writes through (Enter/blur in `'cell'` mode, or per pasted cell) with the single `BstCellEdit` (rowId · columnId · field · old→new value + formatted text · row); emitted from `persist()` downstream of `commitCell`, so it also covers Enter/Tab commit-and-move + paste, while deferred `'row'`/`'batch'` modes still batch through `onSave`. When `enableEditLog` is on and no handler is set, each commit is `console.log`ged instead (handler wins — no double signal). In settings ("Editing", always shown; needs `enableEditing`). Both forward through the adapters (`...rest`) | ✅ done |
| Export — CSV/Excel/print (X1–X3) | `enableExport` | engine | boolean \| `BstExportOptions` | false | custom — **dep-free** `toCsv` / `toXlsx` (store-only ZIP + OOXML, **no exceljs**) / `buildPrintHtml`; `runtime.exportCsv`/`exportExcel`/`printTable`. Scope defaults to **all pages** (pre-pagination); values formatted per cell type (match copy). In settings ("Export", always shown) | ✅ done |
| CSV export (X1) | `enableCsvExport` | engine | boolean | true | custom — sub-toggle of `enableExport`; gates `runtime.exportCsv` + the CSV menu item (RFC-4180, UTF-8 BOM). In settings ("Export", always shown) | ✅ done |
| Excel export (X2) | `enableExcelExport` | engine | boolean | true | custom — sub-toggle; gates `runtime.exportExcel` + the Excel item; a real `.xlsx` (hand-built store-only ZIP + SpreadsheetML, numeric cells typed). In settings ("Export", always shown) | ✅ done |
| Print (X3) | `enablePrint` | engine | boolean | true | custom — sub-toggle; gates `runtime.printTable` + the Print item; opens a print-friendly window (hidden-iframe fallback). In settings ("Export", always shown) | ✅ done |
...
| Set Filter (X4) | `enableSetFilter` (+ `meta.filter`) | engine | boolean | false | custom — Excel-style **checklist of distinct values** per column in the filter row (`BstSetFilter`: search · select-all/clear · counts · (Blanks)). Writes an `{op:'set'}` condition through the `bstCondition` filterFn. Categorical columns (select/multiSelect/radio/boolean) auto-eligible; `meta.filter: 'set'`/`'condition'` forces/opts out. Needs `enableColumnFilters` + `enableColumnFilterRow`. In settings ("Data operations", always shown) | ✅ done |
| Multi-filter (X11) | `enableMultiFilter` (+ array `meta.filter`) | engine | boolean | false | custom — **stack several filter types on one column**. A column opts in with an **array** `meta.filter` (e.g. `['condition','set']`), rendered **stacked** in its filter row; a row must satisfy **all** parts (AND). Stores a compound `{op:'and',conditions}` value the `bstCondition` filterFn understands (`combineFilterConditions`; `FilterConditionGroup`); `BstSetFilter` + the condition input each drive a positional slot. Needs `enableColumnFilters` + `enableColumnFilterRow` (a `'set'` part needs `enableSetFilter`); when off, an array `meta.filter` falls back to its first entry. In settings ("Data operations", always shown). Both skins inherit it | ✅ done |
| Status bar (X5) | `showStatusBar` | chrome | boolean | false | adapter footer — total / filtered row count + `runtime.getSelectionStats()` (sum·avg·min·max·count of the selected range). Selection aggregates need `enableCellSelection`. In settings ("Display") | ✅ done |
...
| Cell notes / comments | `enableNotes` (+ `BstNotesOptions` / `notes` / `onNotesChange` / `onNoteSave`) | engine | boolean \| `BstNotesOptions` | false | custom — attach annotations to individual cells without mutating row data. Cells with notes render a corner marker; hover shows a preview popover; Shift+F2 opens the resizable note editor; context menu has Add/Edit/Delete note. In settings ("Selection & clipboard", always shown). Both skins inherit it | ✅ done |
| Floating Selection Action Bar | `enableFloatingActionBar` (+ `BstFloatingActionBarOptions` / `renderFloatingActions`) | engine | boolean \| `BstFloatingActionBarOptions` | false | custom — floating action toolbar anchored at the bottom-center of the viewport when rows are selected; offers Copy, Delete, Export, Deselect, and custom actions. In settings ("Selection & clipboard"). Both skins inherit it | ✅ done |
| Floating Row Quick-Actions | `enableFloatingRowActions` | engine | boolean | false | custom — floating action pill on row hover with quick actions (Edit, Duplicate, Delete). In settings ("Editing"). Both skins inherit it | ✅ done |
...
| Agent prompt ("copy prompt") | `buildAgentPrompt(corpus)` | tooling | generated markdown | — (generated artifact) | one paste-anywhere briefing that teaches a model this API in a single message — entry points, the `enable*`/`show*` rule, every toggle grouped, all cell types, the dependency traps, and where to look things up. **Generated from the corpus**, so its flag list cannot drift from the release. Rendered to four surfaces from one function: the `bst://prompt` MCP resource · `npx -y @bloomskill/table-mcp prompt` (stdout) · `apps/docs/static/prompt.txt` (served at the site root) · `apps/docs/scripts/prompt.json` → the docs site's **Copy agent prompt** button (`<CopyPromptButton>`, on the landing page + AI Agents page). Emitted by `dump-corpus.mjs`, so `docs:verify` catches a stale copy. Guarded by `agent-prompt.test.ts`: the prompt must name **every** shipped toggle and **no** flag absent from the corpus. Not a settings-sheet toggle (a build artifact, like the MCP corpus) | ✅ done |

| MCP server for AI agents | `@bloomskill/table-mcp` | tooling | MCP stdio server | — (separate package) | `npx -y @bloomskill/table-mcp` — 9 tools (`bst_search_docs` · `bst_get_feature` · `bst_get_cell_type` · `bst_get_api` · `bst_get_example` · `bst_scaffold_grid` · `bst_validate_config` · `bst_detect_version` · `bst_list_versions`), 4 prompts, `bst://` resources. Each tool declares an `outputSchema` (validated structured output); prompt/resource args autocomplete to real names (MCP completions); every flag carries a `since` version and `bst_get_feature` takes `installedVersion` to say if a flag exists in a project's version. Corpus **generated from source** at build time; `src/rules.ts` holds the hand-authored flag-dependency table (parity-tested); **`npm run verify:mcp`** cross-checks the corpus covers every engine capability | ✅ done |
...
> ### ⚠️ "commit and push" = run the whole Definition of Done first — STANDING RULE
> Whenever the user says **"commit and push"** (or just "commit", "push", "ship it", "save it") —
> for **any** change, anywhere in this repo — treat it as a trigger to **first bring every doc into
> sync with the code, in the same commit**, then commit and push. Do **not** commit code alone.
> Before the commit, update whatever the change touched:
> - **`COVERAGE.md`** — flip the capability's status (the source-of-truth matrix).
> - **`docs/backlog.md`** — tick the item / update the snapshot tally.
> - **§12 registry table** (this file) — add or flip the feature's toggle row.
...
8. **Neutral naming** — `npm run verify:naming` green. Bst-Table describes its own capabilities in
   ...
   licensing"), never by comparison to a named vendor. The guard (`scripts/verify-naming.mjs`)
   fails the build on a banned term, so this is enforced, not merely a convention. It scans **both**
   git-tracked sources **and** the npm publish payload (`packages/*/{dist,styles}`) — `tsc` does not
   delete stale outputs, so a reverted source can leave a compiled orphan in `dist` and ship for
   months (this happened: `BstColumnPanel` shipped dead in the engine tarball through v0.41.0).
...
```bash
1. npm run build && npm test && npm run verify:naming && npm run verify:portability   # all green
