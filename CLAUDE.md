...
| Calculated / formula columns (AG17) | `meta.formula` (+ `normalizeFormulaColumns`, `compileFormula`) | engine | `string \| (row, ctx) => value` | — (opt-in by presence) | **computed column** — value derived by a function OR an **Excel-style string** (`'=qty*price'`, `'=IF(age>=18,"a","b")'`, `'=SUM(salary)'`) parsed by a dep-free **safe evaluator** (`formula-expr.ts`: tokenizer + Pratt parser + interpreter, **no `eval`/`Function`**). Field-name refs (`[bracketed]` too), arithmetic/comparison/logical ops, postfix `%`, ~30 functions incl. aggregates over `ctx.rows`. Flows through the accessor so sort/filter/group/`aggregationFn` see it + the cell `type` formats it; errors render `#DIV/0!`/`#VALUE!`/`#NAME?`. Not a settings toggle (per-column config) | ✅ done |
| Runtime formula builder (AG17) | `formulaColumns` (+ `<BstFormulaBuilder>`) | engine | `BstUserFormula[]` | — (opt-in by presence) | runtime end-user-authored computed columns appended to the grid (id · header · Excel-style `expression` · result type). `<BstFormulaBuilder>` (headless, shared by both skins) adds/edits/removes with live preview + validation + a function/field help panel. Stable-accessor-via-ref + data-bump so editing an expression **live-updates** the grid (TanStack caches accessors by id) | ✅ done |
| Formula builder button (AG17) | `showFormulaBuilder` (+ `onFormulaColumnsChange`) | chrome | boolean | false | adapter toolbar **Formula** button → panel hosting `<BstFormulaBuilder>` (MUI `Menu`-style panel + shadcn slide-panel); uncontrolled local state or controlled via the callback. **In the settings sheet** ("Display", `alwaysShow`) like `showFormatBuilder`, so an end-user can switch the Formula button on themselves | ✅ done |
